...
if __name__ == '__main__':
    ...
    log_dirs = glob.glob("{0}/16*".format(options.logdir))
    ...
    for log_dir in log_dirs:
        ...
        for log in logs:
            ...
            with open(log) as f:
                ...
                # -- temperatures --
                charm_log["SiPM_temperature"] = log_parsed["temperatures"]["registers"]["Crate {0} -> RM {1}".format(1,2)]
                for i in [1,2,4]:
                    charm_log["RM_card{0}_temperature".format(i)] = log_parsed["registers"]["registers"]["get HE1-2-{0}-B_SHT_temp_f".format(i)]
                try:
                    charm_log["CU_temperature"] = log_parsed["registers"]["registers"]["get HE1-calib-B_SHT_temp_f"]
                except:
                ...
                charm_log["PeltierVoltage"] = log_parsed["registers"]["registers"]["get HE{0}-{1}-PeltierVoltage_f".format(1,2)]
                charm_log["PeltierCurrent"] = log_parsed["registers"]["registers"]["get HE{0}-{1}-PeltierCurrent_f".format(1,2)]
                charm_log["BVin"] = log_parsed["registers"]["registers"]["get HE{0}-{1}-BVin_f".format(1,2)]
